...
\textbf{Objective.} Ground-truth labeling of measurement errors in pediatric electronic health records (EHRs) is epistemically impossible: once a height is saved, the true value the tape measure would have shown is unrecoverable, and even trained clinicians cannot reliably distinguish pathological growth, measurement noise, and data-entry error in retrospect. Every existing cleaning algorithm therefore relies on synthetic benchmarks or human consensus labels with low inter-rater agreement --- not because the field has been incautious, but because no better evidence base exists. We argue that this identifiability problem is dissolved, not merely managed, by reframing the task: rather than adjudicating historical truth, a prospective alert fires \emph{while the child is still in the clinic}, when remeasurement can recover the true value. We developed and evaluated a prospective remeasurement-alert algorithm that examines each newly entered height against the child's prior trajectory and flags measurements suspicious enough to warrant immediate remeasurement.

\textbf{Materials and Methods.} The alert pipeline combines two hard rules (biologically implausible value thresholds and carry-forward duplicate detection) with five soft detectors operating on the patient's prior history only: short-interval velocity, long-interval velocity, height-for-age z-score change, a streaming LOWESS trajectory-residual check, and a soft HAZ magnitude check. Per-detector confidences are fused by a weighted noisy-OR rule into a single score in [0, 1]; an alert fires when this score clears a tunable threshold (default 0.60). We evaluated the pipeline in streaming mode --- where only prior unflagged history is available, exactly as in live EHR deployment --- on six published Monte Carlo simulated-error datasets from Harrall et al. (600 patients, 17,705 visits), and compared it to streaming reimplementations of the Daymont and Harrall algorithms and to the EPIC 10\% rule deployed in current commercial EHRs. We additionally applied the pipeline to a de-identified EHR extract from the Pediatric Physicians' Organization at Children's (PPOC; 250,588 patients, \(\sim\)6.5M visits) to quantify real-world alert burden.

\textbf{Results.} At the default threshold, the pipeline achieved pooled sensitivity 0.88 and specificity 0.96 across the six benchmarks --- the same synthetic evidence base on which all prior algorithms have been evaluated --- exceeding the streaming Harrall (0.71 / 0.95), streaming Daymont (0.82 / 0.99), and EPIC 10\% (0.76 / 0.33) baselines on sensitivity while remaining competitive on specificity. The threshold traces a smooth precision--sensitivity tradeoff: moving from 0.30 to 0.80 reduces false positives nearly five-fold at a cost of only 2.5\% of true positives. On PPOC, the EPIC 10\% rule would alert on approximately 1.76 million measurements (\(\sim\)27\% of all visits); the proposed pipeline alerts on 25\% at the default threshold.

\textbf{Discussion.} The definitionally correct external validation for a prospective alerting system is itself prospective: deploy the algorithm, remeasure when it fires, and recover ground truth while the child is present. We describe a proposed evaluation protocol and regard prospective clinical deployment as a necessary next step. The simulated benchmark used here is not a limitation specific to this work --- it is the evidentiary ceiling available to the entire field.

\textbf{Conclusion.} A multi-detector, confidence-fused prospective alert pipeline identifies pediatric height measurements warranting remeasurement with performance exceeding the EPIC 10\% rule and two leading retrospective algorithms in streaming use. By targeting the moment of entry rather than historical records, the method converts an irresolvable retrospective inference problem into a recoverable prospective action.

\section{Background}\label{background}

The central obstacle in pediatric growth-data quality research is epistemological: the true value of a clinical measurement is permanently unrecoverable once it has been saved to the electronic health record. A recorded height of 112 cm for a seven-year-old may reflect a genuine measurement, a transcription error, a unit confusion, or a carry-forward duplication of a prior visit's value --- and these possibilities cannot be reliably distinguished from the longitudinal record alone, even by trained clinicians with full access to the child's history. This is not a contingent limitation of existing methods; it is a structural property of retrospective EHR data. As Daymont noted, the same record contains both plausible outliers (true extreme values that look like errors) and implausible inliers (erroneous values that look ordinary against the population distribution but are wrong for the individual child) [7], and no retrospective procedure can separate these categories without external ground truth that does not exist.
...
\item
  \textbf{EPIC 10\% rule.} A new measurement is flagged when its height differs from the most recent un-flagged previous height by more than 10\%, mimicking the rule deployed in the EPIC EHR. Flagged points are removed from history for subsequent checks.
\end{itemize}
...
The tradeoff is asymmetric. Between thresholds 0.30 and 0.80, sensitivity declines only modestly (0.89 \(\to\) 0.87); the false-positive count falls almost five-fold (1,611 \(\to\) 347) for the cost of 54 true positives (2.5\% relative loss in TP). Only beyond threshold 0.80 does sensitivity drop steeply. The implication for deployment is that a clinic with low tolerance for false alarms can move the threshold up to 0.80 with little loss of error capture, and only at the most conservative end (\(\ge\) 0.90) does the algorithm begin to miss meaningfully more true errors.
...
The EPIC 10\% rule alerts on roughly 1.76 million measurements (\(\approx\) 27\% of all visits). Our pipeline produces an order-of-magnitude smaller alert burden across the entire threshold range, and the burden is tunable: a deploying clinic can dial the threshold down for higher sensitivity or up for a lighter operational footprint without re-engineering the alert. This is the practical payoff of the trajectory-aware, confidence-fused design --- the same algorithm that achieves higher sensitivity than EPIC on simulated errors also generates a small fraction of EPIC's alerts on real EHR traffic.

\section{Discussion}\label{discussion}

\subsection{Why a prospective frame}\label{why-a-prospective-frame}

The most important claim in this paper is not about algorithm performance --- it is about problem formulation. The retrospective growth-data cleaning literature, for all its sophistication, operates under an epistemological constraint it has not fully confronted: the ground truth it seeks to recover does not exist in the data, has not existed since the measurements were saved, and cannot be reconstructed by any algorithm operating on the historical record alone. Simulated benchmarks and human consensus labels are not approximations to a better evaluation that is merely difficult to obtain --- they are the best evaluation available, period, for any retrospective method.

Acknowledging this constraint changes what counts as a valid evaluation design. A prospective alerting system --- one that fires \emph{before} a measurement is saved, while the child can still be remeasured --- is the only task formulation for which external ground truth is recoverable. When the algorithm fires and the nurse takes a second measurement, the pair (original, remeasured) is the ground truth. This is not merely a practical convenience; it is the definitionally correct evaluation paradigm for this class of algorithm. Prospective clinical deployment is therefore not a direction for future work in the ordinary sense --- it is the experiment that will provide the first true external validation of any algorithm in this space.

The simulated benchmark used in this paper is the same evidentiary foundation on which the entire retrospective cleaning literature rests. Our evaluation is not weaker than prior work's evaluation by virtue of using simulation; it uses identical evidence while making a more honest claim about what that evidence can and cannot establish. The performance results in Section~3 should therefore be read as establishing that the algorithm meets the bar that prior algorithms in this space have been evaluated against --- a necessary condition for serious consideration --- rather than as a definitive demonstration of clinical superiority. Section 4.4 outlines the experiment that would establish the latter.

\subsection{Comparison to EPIC 10\% rule}\label{comparison-to-epic-10-rule}

The EPIC 10\% rule's catastrophically low specificity (0.33 pooled) reflects the fundamental mismatch between a fixed proportional threshold and pediatric growth physiology. Children grow at strongly age-dependent velocities, and 10\% per visit is easily exceeded by ordinary growth in infants (\(\approx\) 50\% per year), early adolescents during peak height velocity, and any child with intervisit gaps of more than a year. Our results suggest that even a small amount of trajectory awareness --- at minimum, age- and sex-adjusted velocity z-scores --- could substantially reduce alert burden in EHRs that currently deploy the 10\% rule, without sacrificing the sensitivity to gross errors that motivated the rule in the first place.

\subsection{Limitations}\label{limitations}

The primary limitation of this evaluation --- that it relies on simulated data --- is not a contingent weakness of this paper but a structural feature of the entire field. It is worth stating this precisely: no algorithm for retrospective or prospective detection of pediatric EHR measurement errors has ever been evaluated against objectively recoverable ground truth on real EHR data, because such ground truth does not exist once a measurement is saved. The simulated and consensus-labeled benchmarks used across this literature are not approximations to a gold standard that is merely hard to obtain --- they are the ceiling of evidence available to any method in this space. This paper uses the same ceiling. What distinguishes the prospective framing is that it creates, for the first time, a path to genuine external validation: prospective deployment, in which an alert triggers remeasurement before the original value is committed, and the pair of measurements resolves the ambiguity in real time.
...
The arguments in Section~4.1 and Section~4.3 lead to a specific empirical commitment: the methodologically appropriate external validation of a remeasurement-alert system is not another retrospective benchmark but a \emph{prospective} in-clinic study in which the algorithm fires, the child is remeasured before they leave the room, and the original--remeasurement pair is recorded as ground truth. This is the only setting in which the counterfactual ("what would a careful measurement have read?") is empirically recoverable rather than inferred. We outline below a protocol intended to support a multi-site evaluation.
...
The central contribution of this paper is a reframing of the growth-data quality problem, not an incremental improvement in algorithm design. Retrospective adjudication of EHR measurement quality is epistemologically constrained: ground truth is unrecoverable, and any algorithm --- however sophisticated --- can only be evaluated against synthetic errors or human consensus labels with limited agreement. A prospective alert, by contrast, fires at the moment of entry, while the child and clinician are present and remeasurement is possible. This reframing converts an irresolvable inference problem into a recoverable action problem, and it creates, for the first time, conditions under which a growth-data quality algorithm can be evaluated against true external ground truth in a clinical deployment setting.

Within this framing, the multi-detector confidence-fused pipeline presented here achieves pooled sensitivity 0.88 and specificity 0.96 on the Harrall Monte Carlo benchmark in streaming mode --- the same evidence base on which all prior algorithms have been evaluated --- exceeding the streaming Harrall and Daymont algorithms on sensitivity and dramatically exceeding the EPIC 10\% rule on specificity. The alert threshold is tunable, the operational burden on real-world EHR traffic is a small fraction of the EPIC rule's, and the algorithm requires only the patient's prior history, making it suitable for deployment in any EHR system with longitudinal growth records.
